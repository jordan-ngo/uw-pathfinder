...
public class SimpleSetTest {
  ...
  public void testSize() {
    // TODO: implement this
    assertTrue(R.size()== R1.size());
    assertTrue(R.size()==R.size());
    assertTrue(S1.size()==S2.size());
    assertTrue(S1.size()== S2.size());
    assertFalse(S.size()==R.size());
    assertEquals(S1.size(), 1, 5);
    assertEquals(R.size(), Float.POSITIVE_INFINITY, 5);
    assertEquals(S12.size(), 2.0, 5);
    assertEquals(S.size(), 0, 5);
